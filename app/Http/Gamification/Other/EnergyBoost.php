...
use App\Models\DailyEnergy as Bonus;

class EnergyBoost extends Component
{
    ...
    public function claimDailyEnergy(int $energySize = 1)
    {
        $user = $this->user;
        ...
        if($this->isAvail){
            $this->AddPropValue($user, 'health', $energySize);
            $this->AddPropValue($user, 'exps', 1);
            $this->HealthSuccessAlert($energySize);
        }

        $this->isAvail = false;

        $this->ApplyBadgePerBonus($user->id, 'streak');

        $this->dispatch('refreshTops', name:'energy');
    ...
    {
        return view('gamification.other.energy-boost');
    }
