<div class="{{ ($invCorrectCount == $correctCount) && ($userQuest->amount < $normalHealth) ? '' : 'd-none' }}">
	<hr>
	<h6 class="font-weight-normal">Recovery</h6>
	<div class="row justify-content-center overflow-auto gx-2" style="max-height: 20em;">
		@foreach($revives as $item)
			<div wire:loading.remove wire:target="search" class="col-auto">
				<div wire:key="revive-{{$loop->iteration}}" class="icon icon-inv icon-shape bg-dark shadow text-center text-white border-radius-xl position-relative cursor-pointer m-1" wire:click.prevent="openReviveModal({{ $item->damage }}, '{{$item->name}}', '{{$item->icon}}')" title="Revive {{ $item->damage }} patient health">
					<img width="30" height="auto" src="/assets/svg/revive/{{$item->icon}}.png" class="d-block mx-auto mt-2"/>
					<div class="mx-auto text-center text-white text-xs pt-1">
						{{ $item->name }}
